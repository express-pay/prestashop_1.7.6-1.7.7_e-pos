...
class ExpressPayEposRedirectModuleFrontController extends ModuleFrontController
{
	public function initContent()
	{
		parent::initContent();

		$token      = Tools::safeOutput(Configuration::get('EXPRESSPAY_EPOS_TOKEN'));
		$url        = Tools::safeOutput(Configuration::get('EXPRESSPAY_EPOS_TESTING_MODE'))  ? Tools::safeOutput(Configuration::get('EXPRESSPAY_EPOS_TEST_API_URL'))
			: Tools::safeOutput(Configuration::get('EXPRESSPAY_EPOS_API_URL'));

		$id_cart  = $this->context->cart->id;

		$cart       = new Cart($id_cart); // Объект корзины
		$expressPay = Module::getInstanceByName('expresspayepos'); //Объект ExpressPay
		...
		$isNameEditable = Configuration::get('EXPRESSPAY_EPOS_ALLOW_CHANGE_NAME') == false ? 0 : 1;
		$isAddressEditable = Configuration::get('EXPRESSPAY_EPOS_ALLOW_CHANGE_ADDRESS') == false ? 0 : 1;
		$isAmountEditable = Configuration::get('EXPRESSPAY_EPOS_ALLOW_CHANGE_AMOUNT') == false ? 0 : 1;

		$emailNotification = $customer->email;
		$smsPhone = $address->phone;

		$requestParams = array(
			"serviceid"			=> Configuration::get('EXPRESSPAY_EPOS_SERVICE_ID'),
			"accountno"  		=> $accountNo,
			"amount"     		=> $amount,
		...
		$signature = $this->computeSignature($requestParams, Configuration::get('EXPRESSPAY_EPOS_SEND_SECRET_WORD'), 'add-web-invoice', $expressPay, $token);

		$requestParams['signature'] = $signature;
		//}

		$response = $this->sendRequestPOST($url . 'web_invoices', $requestParams, $expressPay);
		...
		if (isset($response['ExpressPayInvoiceNo'])) {
			$expressPay->log_info('initContent', 'InvoiceNo - ' . $response['ExpressPayInvoiceNo']);
		} else if (isset($response['Message'])) {
			$this->errors[] = $this->trans($response['Message'], $response, 'Modules.ExpressPayEpos.Shop');

			$expressPay->log_error('initContent', 'ERROR MESSAGE - ' . $response['Message']);
		}


		Tools::redirect('index.php?controller=order-confirmation&id_cart=' . (int)$cart->id . '&id_module=' . (int)$this->module->id . '&id_order=' . $this->module->currentOrder . '&key=' . $customer->secure_key . '&ExpressPayInvoiceNo=' . $response['ExpressPayInvoiceNo']);
	}

	public function sendRequestPOST($url, $params, $expressPay)
	{
		try {
			$ch = curl_init();
			...
			$expressPay->log_error('receipt_page', 'Get response; ORDER ID - ' . $params['AccountNo'] . '; RESPONSE - ' . $response, $e);
		}
	}
